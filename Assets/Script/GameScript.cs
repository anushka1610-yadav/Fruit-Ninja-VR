...
public class GameScript : MonoBehaviour
{
    ...
    IEnumerator InstantiateRandomly()
    {
        while (true)
        {
            Transform randomPoint = points[Random.Range(0, points.Length)]; // Random point from the array
    ...
    IEnumerator InstantiateAndMove(GameObject objectToInstantiate, Transform targetPoint)
    {
        Vector3 startPosition = new Vector3(targetPoint.position.x, 1, targetPoint.position.z); // Start position below the plane
        ...
        instantiatedObject.transform.position = targetPositionUp; // Ensure it reaches the target position

        // Wait for a moment at the top
        yield return new WaitForSeconds(0.1f);

        // Move down
        Vector3 endPosition = new Vector3(targetPoint.position.x, 0.15f, targetPoint.position.z); // End position on the plane
